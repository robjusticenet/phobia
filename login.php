	...
	if($_POST["processlogin"] == 3) { // if user requests forgotten password
	    ...
	    if (check_email($email)){
			...
			if($user){
				...
				$body = sprintf($main_smarty->get_config_vars("KLIQQI_PassEmail_Body"),$main_smarty->get_config_vars("KLIQQI_Visual_Name")); 
				$body .="\n \n";
				$body .= $my_base_url . $my_kliqqi_base . '/recover.php?id=' . base64_encode($username). '&n=' . time();
