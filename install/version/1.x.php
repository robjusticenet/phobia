<?php
// This file contains code used to upgrade between 1.x Kliqqi versions
...
$old_version = str_replace('.', '' , $kliqqi_version);
$new_version = '122';
...
if ($old_version < $new_version) {
	...
	echo '<li>Performing one-time Kliqqi 1.X Upgrade</li><ul>';
	...
	if (count($result) == 0) {
		$db->query("INSERT INTO `" . table_config . "` ( `var_id` , `var_page` , `var_name` , `var_value` , `var_defaultvalue` , `var_optiontext` , `var_title` , `var_desc` , `var_method` , `var_enclosein` )VALUES (NULL, 'Misc', 'user_language', '0', '0', '1 = yes / 0 = no', 'Select Language', 'Allow users to change Kliqqi language', 'define', '\'');");
	}
	...
	if (!$fieldexists) {
		$sql = "ALTER TABLE  `kliqqi_links` ADD  `link_group_status` SET(  'new',  'published',  'discard' ) DEFAULT 'new' NOT NULL ;";
		$db->query($sql);
	}
	...
	if (!$tableexists) {
		...
		$db->query("INSERT INTO `".table_widgets."` VALUES (4, 'Kliqqi CMS', 0.1, 0, 'kliqqi_cms', 1, 'right', 5, '')");
		$db->query("INSERT INTO `".table_widgets."` VALUES (5, 'Kliqqi News', 0.1, 0, 'kliqqi_news', 1, 'right', 6, '')");
...
/* Redwine: using php explode() instead of the deprecated split() which generates warnings. */
	...
	$sql = "UPDATE `" . table_misc_data . "` SET `data` = '1.2.2' WHERE `name` = 'kliqqi_version';";
	$db->query($sql);
