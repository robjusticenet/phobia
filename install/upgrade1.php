...
if($language == '' && $_POST['submit'] == ''){
	...
	if(strpos($data, '!--Kliqqi Language Select-->') > 0){
		...
		echo 'We just tried to connect to Kliqqi.com to get all of the language files available for installation, but there was a problem. That\'s okay because we can continue by using the local English version.';
		echo '</div>';
...
if (!$errors) {
	...
	// Get Kliqqi CMS Version as $old_version
...
// echo $languageContent;
	...
	if (!$tableexists) {
		...
		include_once('version/3.0.0.php');
