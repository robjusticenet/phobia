...
class Link {
	...
	/* Redwine: initialized a new variable $is_rtl to hold the value 0 or 1 when returned from the script that checks whether the content is left-to-right or right-to-left language. */
	var $is_rtl = 0;
	function get($url) {
		$url=trim($url);
...
/* Redwine: Roles and permissions and Groups fixes */	
		...
		$smarty->assign('pagename', 'pagename'); // Redwine: ???? this!!! to check.
	...
	function truncate_content(){
		if(utf8_strlen($this->content) > StorySummary_ContentTruncate){
			 ...
			 if(Auto_scroll==true){
				...
				$content.="<div class=\"read_more_article\" storyid=\"".$this->id."\" > ".$main_smarty->get_config_vars('KLIQQI_Visual_Read_More')."</div>" ;
